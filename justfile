dev:
    fd 'go|templ' | entr -r bash -c 'just templ && just tailwind && go build -o ./pyramid-sebastix && godotenv ./pyramid-sebastix'
...
build cc='musl-gcc' arch='amd64': templ tailwind
    ...
    CC={{cc}} CGO_ENABLED=1 GOARCH={{arch}} GOOS=linux go build -tags=libsecp256k1 -ldflags="-X main.currentVersion=$VERSION -linkmode external -extldflags \"-static\"" -o ./pyramid-sebastix
